...
>
1. 설계상 매개변수 이름을 보존하는 **record class**를 사용합니다.
1. `javac`를 사용하는 경우 **`-parameters` 옵션**으로, `kotlinc`를 사용하는 경우 **`-java-parameters` 옵션**으로 컴파일합니다.
    **Spring Boot**를 사용하는 경우 [Spring Boot Gradle 플러그인](https://docs.spring.io/spring-boot/gradle-plugin/reacting.html#reacting-to-other-plugins.java) 또는 [Spring Boot Maven 플러그인](https://docs.spring.io/spring-boot/maven-plugin/using.html)을 사용하면 이 옵션이 자동으로 활성화됩니다.
1. **`@ConstructorProperties`** 애너테이션을 적용하여 매개변수 이름을 명시적으로 선언합니다.
    **Lombok**을 사용 중이고 생성자가 `@AllArgsConstructor`와 같은 롬복 애너테이션에 의해 생성되는 경우 `lombok.anyConstructor.addConstructorProperties = true` 옵션을 활성화하면 자동으로 애너테이션을 추가할 수 있습니다.
