...
`@ValueAutoSource` 어노테이션은 `@ValueSource`와 `@AutoSource`의 기능을 결합하여 테스트 인수를 생성하는 더 유연한 방법을 제공합니다. 구체적으로, 첫 번째 매개변수에 미리 정의된 값을 할당하고 나머지 매개변수에 대해 임의의 값을 생성합니다.

## 사용 방법

`@ValueAutoSource` 어노테이션을 사용하려면 테스트 메서드 위에 어노테이션을 추가하고 `@ValueSource`와 마찬가지로 `strings`, `ints` 등의 속성을 사용하여 첫 번째 매개변수의 값을 지정합니다.
